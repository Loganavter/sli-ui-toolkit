...
class DialogActionBar(QWidget):
    def __init__(
        ...
        *,
        ...
        self._layout.addWidget(self.primary_button)
        self._layout.addWidget(self.secondary_button)

        self.extra_buttons: list[Button] = []

    def add_extra_button(self, text: str, *, min_size: tuple[int, int] = (100, 32)) -> Button:
        button = Button(text=text, variant="surface")
        button.setMinimumSize(*min_size)
        insert_index = self._layout.indexOf(self.primary_button)
        self._layout.insertWidget(insert_index, button)
