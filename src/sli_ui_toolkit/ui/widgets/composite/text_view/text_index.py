...
def segment_bounds_at_offset(
    ...
    possible (same idea as caret-at-end for word selection). Multi-line
    segments — fenced code blocks — resolve to the CURRENT LINE instead of
    the whole block: triple-clicking code selects the line, like the code
    editor, not the entire fence.
    """
    ...
    for seg in index.segments:
        if seg.start <= offset < seg.end:
            return _segment_or_line_bounds(index, seg, offset)
...
def _segment_or_line_bounds(
    index: DocumentTextIndex,
    seg: TextSegment,
    offset: int,
) -> tuple[int, int]:
    """Segment bounds at ``offset``; for multi-line segments (code blocks)
    the current line's bounds instead — triple-click selects the line, not
    the fence. Multi-line segments only ever come from ``add_code`` (no
    other block type emits ``\\n`` inside a segment)."""
    text = index.text
    if "\n" not in text[seg.start:seg.end]:
        return seg.start, seg.end
    offset = max(seg.start, min(offset, seg.end - 1))
    start = offset
    while start > seg.start and text[start - 1] != "\n":
        start -= 1
    end = offset
    while end < seg.end and text[end] != "\n":
        end += 1
    return start, end
...
class _TextIndexBuilder:
    ...
    def add_code(self, block: CodeBlock, block_index: int) -> None:
        self.add_block_separator()
        start = self._begin_segment()
        role = TextRole(TextRoleKind.BODY)
        style = CharStyle(InlineKind.CODE)
        for line_index, line in enumerate(block.lines):
            if line_index:
                self._append_char("\n", style=style, role=role)
            self._append_plain(line, style=style, role=role)
    ...
    def add_table_cell(
        ...
        *,
        ...
        """One table cell (header or body); ``cell_index`` enumerates cells
        left-to-right, top-to-bottom (header first, then each row)."""
        self.add_block_separator()
        start = self._begin_segment()
        self._append_spans(spans)
...
def build_text_index(blocks: tuple[HelpBlock, ...]) -> DocumentTextIndex:
    ...
    for block_index, block in enumerate(blocks):
        if isinstance(block, HeadingBlock):
            ...
            builder.add_code(block, block_index)
            ...
            all_rows = ([block.header] if block.header else []) + list(block.rows)
            ...
            for row_index, row in enumerate(all_rows):
                for col_index, cell in enumerate(row):
                    builder.add_table_cell(
                        spans=cell,
                        block_index=block_index,
                        cell_index=row_index * col_count + col_index,
                    )
