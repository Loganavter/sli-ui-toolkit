...
from PySide6.QtGui import QFont, QFontMetrics, QPixmap, QTextOption
...
from sli_ui_toolkit.ui.widgets.composite.text_view.layout.segment_map import (
    build_segment_map,
)
from sli_ui_toolkit.ui.widgets.composite.text_view.layout.text_layout import (
    build_text_layout,
    styles_from_spans,
    text_layout_height,
)
...
class _LayoutBuilder:
    ...
    def _body_font(self) -> QFont:
        # Design px — ui_font scales by the UiScale factor exactly once (and
        # pins the UI family), so the whole document grows with the interface
        # scale instead of freezing at its design size.
        return ui_font(pixel_size=BODY_FONT_PX)
    ...
    def _heading_font(self, level: int) -> QFont:
        ...
        font = ui_font(pixel_size=sizes.get(level, H3_FONT_PX))
        font.setBold(True)
        return font

    def _caption_font(self) -> QFont:
        return ui_font(pixel_size=CAPTION_FONT_PX)

    def _code_font(self) -> QFont:
        return ui_font(pixel_size=BODY_FONT_PX, family="monospace")
    ...
    def _layout_segment(
        ...
        *,
        ...
        if not text:
            return 0.0
        # QTextLayout treats "\n" as a *soft* break: the newline only
        # produces a line when wrapping is forced, otherwise the whole text
        # lands in one line with the newlines swallowed. Split the segment
        # on "\n" and emit one fragment per line so hard breaks always hold
        # (code blocks keep their line structure). Empty lines reserve the
        # font's line height without a fragment.
        line_height = float(QFontMetrics(base_font).lineSpacing())
        running = 0.0
        start = 0
    ...
    def _layout_part(
        ...
        *,
        ...
        rect = QRectF(x, self.y + y_offset, max_width, natural)
        self.text_fragments.append(
            TextFragment(
                rect=rect,
                global_start=segment_start,
                global_end=segment_start + len(text),
                layout=layout,
                links=links,
            )
        )
    ...
    def _layout_code(self, block: CodeBlock) -> None:
        """Fenced code block: monospace text inside a shaded rounded box
        (``help.code.background`` token), fence language label in the box
        header. Non-wrapping layout so indentation and line structure
        survive; long lines wrap anywhere rather than being clipped."""
        segment = self._segment_for(self._block_index(block))
        if segment is None:
            return
        language = block.language
        header_h = CODE_HEADER_H if language else 0.0
        box_top = self.y
        self.y = box_top + header_h + CODE_PAD_Y
        frag_start = len(self.text_fragments)
        text_h = self._layout_segment(
            segment,
            x=CODE_PAD_X,
            max_width=max(1.0, self.width - 2 * CODE_PAD_X),
            base_font=self._code_font(),
            wrap_mode=QTextOption.WrapMode.WrapAtWordBoundaryOrAnywhere,
            # The box is the fence's background — no per-character CODE tint,
            # which would darken the glyphs a second time (and `setFormats`
            # on a laid-out QTextLayout breaks its drawing).
            code_tint=False,
        )
        ...
        box = QRectF(
            ...
            box_top,
            self.width,
            header_h + text_h + 2 * CODE_PAD_Y,
        )
        for index in range(frag_start, len(self.text_fragments)):
            self.text_fragments[index] = replace(
                self.text_fragments[index],
                code=True,
                code_box=box,
                code_language=language,
            )
        self.y = box_top + header_h + text_h + 2 * CODE_PAD_Y
        self._finish_block()
    ...
    def _layout_table(self, block: TableBlock) -> None:
        """Bordered N-column table with an optional bold header row.

        Column widths come from the widest cell per column (header cells
        measured in the bold body font). Every column except the last is
        capped at 60% of the remaining width; the last column takes the
        rest — the same rule the two-column label/value table used, so the
        Image Properties look is unchanged.
        """
        ...
        block_index = self._block_index(block)
        font = self._body_font()
        bold_font = self._bold_body_font()
        body_metrics = QFontMetrics(font)
        bold_metrics = QFontMetrics(bold_font)

        col_count = max(len(row) for row in all_rows)
        col_widths = [0.0] * col_count
        for row_index, row in enumerate(all_rows):
            metrics = bold_metrics if row_index == 0 and block.header else body_metrics
            for col_index, cell in enumerate(row):
                col_widths[col_index] = max(
                    col_widths[col_index],
                    float(metrics.horizontalAdvance(spans_to_plain(cell))),
                )

        # Divider x-positions between columns; the last column takes the rest.
        col_xs: list[float] = []
        cursor = 0.0
        for col_index in range(col_count - 1):
            col_w = min(
                col_widths[col_index],
                max(1.0, (self.width - cursor) * 0.6),
            )
            cursor += col_w + 2 * TABLE_CELL_PAD_X
            col_xs.append(cursor)

        table_top = self.y
        row_ys: list[float] = []
        for row_index, row in enumerate(all_rows):
            row_top = self.y
            is_header = row_index == 0 and block.header
            row_h = 0.0
            for col_index, cell in enumerate(row):
                list_item_index = row_index * col_count + col_index
                seg = self._segment_for(
                    block_index, list_item_index=list_item_index
                )
                if seg is None:
                    continue
                x = (
                    TABLE_CELL_PAD_X
                    if col_index == 0
                    else col_xs[col_index - 1] + TABLE_CELL_PAD_X
                )
                if col_index < col_count - 1:
                    max_w = col_xs[col_index] - TABLE_CELL_PAD_X - x
                else:
                    max_w = self.width - TABLE_CELL_PAD_X - x
                self.y = row_top + TABLE_CELL_PAD_Y
                h = self._layout_segment(
                    seg,
                    x=x,
                    max_width=max(1.0, max_w),
                    base_font=bold_font if is_header else font,
                )
                row_h = max(row_h, h)
            row_h += 2 * TABLE_CELL_PAD_Y
