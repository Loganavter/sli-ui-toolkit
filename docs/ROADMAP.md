...
- ~~Audit keyboard navigation and focus states across buttons, comboboxes, flyouts, dialogs, and composite widgets.~~ *(full audit in `KEYBOARD.md`; 0.2.7 composite pass added `TimelineWidget` Delete/arrow/Home/End handling and re-classified `EditableListItem` to OK; `InstancesCounterButton` documented as deferred / mouse-only by design)*
